...
For self penalty, use the snippet from RDM/AlexNet_BN_LSTM_self_sup.py
The following script replicates the results in the main text.
For replicating the results in the SI, use the snippet from RDM/AlexNet_BN_LSTM_sup_2.py
"""
...
class AlexNet_BN_LSTM_wrapper(nn.Module):
    def __init__(self, time_steps=150, dt=1/75.0, sigma=2.0):
        ...
        self.threshold = torch.nn.Parameter(torch.tensor(0.2))
        self.dt = dt
        self.sigma = sigma
...
def main():
    ...
    optimizer = optim.Adam(model.linear.parameters(), lr=1e-5)
    optimizer.add_param_group({'params': model.w, 'lr': 1e-4})
    optimizer.add_param_group({'params': model.b, 'lr': 1e-4})
    ...
    train_model(model, optimizer, histogram, human_rt_dist_train, human_rt_dist_test,
            ...
            'BATCH_SIZE': 1024,
            'dataset_size': 2048,
            'model_name': f'AlexNet_BN_LSTM_BP/human_fit',
