"""
The following script trains the RTify module with self-penalty on the model
For direct fitting, use the snippet from AlexNet_BN_LSTM_sup.py
The following script replicates the results in the main text.
For replicating the results in the SI, use the snippet from RDM/AlexNet_BN_LSTM_self_sup_2.py
It turns out RDM/AlexNet_BN_LSTM_self_sup_2.py might be more stable and easier to train,
but the following script yields better results.
"""
...
class AlexNet_BN_LSTM_wrapper(nn.Module):
    def __init__(self, time_steps=150, dt=1/75.0, sigma=2.0):
        ...
        self.w = nn.Parameter(torch.ones((1, time_steps)))
    ...
    def forward(self, lstm_out):
        ...
        soft_index = torch.sigmoid((decision_time.unsqueeze(1) - torch.arange(self.time_steps, device=decision_time.device) + 0.5) * self.sigma)
        logit_trajectory = self.fc1(lstm_out).squeeze() * self.fc2.weight
        decision_logits = (logit_trajectory * soft_index).sum(dim=1, keepdim=True) + self.fc2.bias

        return decision_logits, decision_time


def train_model(model, criterion, optimizer, histogram, human_rt_dist_train, human_rt_dist_test, config, coherence_list=None, lr_scheduler=None):
    ...
    for epoch in tqdm(range(config['num_epochs'])):
        ...
        for i_coh, coh in enumerate(coherence_list):
            ...
            loss = criterion(outputs, direction_list[i_coh][indices].to(device)) + (outputs * (2*direction_list[i_coh][indices].to(device)-1) * decision_time).mean() * config['lambda']
...
def main():
    ...
    optimizer.add_param_group({'params': model.threshold, 'lr': 1e-5})
    optimizer.add_param_group({'params': model.w, 'lr': 1e-6})
    optimizer.add_param_group({'params': model.b, 'lr': 1e-6})

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.load_state_dict(torch.load('ckpt/AlexNet_BN_LSTM_norm/epoch_100.pth', map_location=device)['state_dict'], strict=False)
    ...
    # Note since it is a self-penalty case, human_rt_dist_train is actually not used
    train_model(model, criterion, optimizer, histogram, human_rt_dist_train, human_rt_dist_test,
                lr_scheduler=None, config={
                'num_epochs': 20000,
                'BATCH_SIZE': 1024,
                'dataset_size': 2048,
                'lambda': 0.0035,
                'model_name': f'AlexNet_BN_LSTM_BP/self_penalty',
                'outputs_path': 'ckpt/AlexNet_BN_LSTM_norm/backbone_outputs_epoch_100_dynamic.pth'})
