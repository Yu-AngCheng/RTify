...
For direct fitting, use the snippet from AlexNet_BN_LSTM_sup_2.py
The following script replicates the results in the SI.
For replicating the results in the main text, use the snippet from RDM/AlexNet_BN_LSTM_self_sup.py
"""
...
def main():
    ...
    # Technically, the module is overparameterized, but setting apart the w and threshold and with a higher lr can help
    # fix the scale issue to prevent the decision time from being too large or too small
    optimizer = optim.Adam(model.linear.parameters(), lr=1e-6*4)
    optimizer.add_param_group({'params': model.w, 'lr': 1e-3})
    optimizer.add_param_group({'params': model.b, 'lr': 1e-6})
    optimizer.add_param_group({'params': model.threshold, 'lr': 1e-3})
    ...
    train_model(model, criterion, optimizer, histogram, human_rt_dist_train, human_rt_dist_test,
                ...
                'dataset_size': 2048,
                'lambda': 0.0027,
                'model_name': f'AlexNet_BN_LSTM_BP_2/self_penalty',
