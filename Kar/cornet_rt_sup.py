'''
The following code is used to train the CORnet model with the human RT as the target.
'''
...
def train_model(model, optimizer, train_dataloader, test_dataloader, config, lr_scheduler=None):
    ...
    for epoch in tqdm(range(config['num_epochs'])):
        ...
        for label, rt, rnn_preds, hidden_state in train_dataloader:
            ...
            loss = NegativePearsonCorrelationLoss()(decision_time, rt.to(device))
            stats.pearsonr(decision_time.detach().cpu().numpy(), rt.detach().cpu().numpy())
            loss.backward()
            optimizer.step()
    ...
    # Test the model
    with torch.no_grad():
        fitted_model_rt, human_rt = evaluate_model(model, test_dataloader)
